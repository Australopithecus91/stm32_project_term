...
void LCD_Data(uint8_t dt) // функция для передачи данных
{
	rs1_set();
	LCD_Write_Data(dt>>4);
	e1_set();
...
void LCD_Clear (void) // функция очистки дисплея
{
	LCD_Command(0x01); // убираем мусор
	HAL_Delay(2); // задержка 2 микросекунд
}
//---------------------------------------------------------------------//
 void LCD_Send_Char(char ch)// функция вывода символов
 {
	LCD_Data ((uint8_t)ch);
	delay_lcd();
 }
//---------------------------------------------------------------------//
 ...
 ///---------------------------------------------------------------------//
 void LCD_String (char* sym) //функция вывод строк
 {
	uint8_t in = 0; // счетчик
	while(sym[in]!=0)
	{
		LCD_Data(sym[in]);
		delay_lcd();
